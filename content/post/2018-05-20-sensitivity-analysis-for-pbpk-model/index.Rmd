...
summary: Applying the Morris screening method to exam the parameter sensitivity in physiological-based pharmacokinetic model (with R deSolve package)
...
In my previous post, I used python [**SALib**](https://nanhung.rbind.io/post/2017-08-29-salib/) to conduct the Sobol sensitivity analysis for the simple example (Ishigami function). Now, I want to take the more complicated case to understand how to apply sensitivity analysis in a dynamic model. 
...
It is an opportunity to apply the sensitivity analysis for this model. Therefore, I choose the elementary effect-based Morris screening method ([**Morris, 1991**](https://www.tandfonline.com/doi/abs/10.1080/00401706.1991.10484804)), which is also the advance local sensitivity analysis that change one-factor-at-a-time (OAT) to investigate the parameter impact on the model output. 
...
To investigate the parameter effect on the PBPK model outputs, we used the following visualization methods for all parameters and the most influential parameter `Pct_Deadspace` and less influential parameter `PC_pp`.   
