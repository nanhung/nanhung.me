...
{{% alert warning %}}
Be sure you're in the right place. Check `.Platform$OS.type == "windows"` first! Linux or MacOS user can read [**this instruction**](https://nanhung.rbind.io/post/mcsim-under-r-unix/).
{{% /alert %}}
...
In this case, we don't need to load libSBML. Therefore, check the `config.h` file in mod folder. We can see the line 26 in the config.h file like [THIS](https://github.com/rbind/nanhung.me/blob/master/content/post/mod/config.h ). The `#define HAVE_LIBSBML 1 */` had been disabled due to there is the "/*" at the start of this line. It can prevent the problem when compile to the mod.exe file. 
(Thanks **Yu-Sheng Lin** for provding this issue; Mar. 23, 2018).
